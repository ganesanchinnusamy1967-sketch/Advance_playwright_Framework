...
export class InventoryPage extends BasePage {
    ...
    async productNames(): Promise<string[]> {
        return this.util.getAllTexts(this.itemNames);
    }
    ...
    async addToCart(id: string): Promise<void> {
        await this.util.click(this.addBtn(id));
    }

    async removeFromCart(id: string): Promise<void> {
        await this.util.click(this.removeBtn(id));
    }

    async openCart(): Promise<void> {
        await this.util.click(this.cartLink);
        await this.page.waitForLoadState('domcontentloaded');
    }

    async openItem(id: string): Promise<void> {
        await this.util.click(this.page.locator(`[data-test="item-${id}-title-link"]`));
        await this.page.waitForLoadState('domcontentloaded');
    }
